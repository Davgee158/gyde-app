...
const SetPinPage = () => {
    ...
    const handleSubmit = () => {
        ...
        navigate("/confirmpin", { state: { pin: pinCode } })
    }
    ...
    style={{
        ...
        <div className='w-full absolute top-[10rem] md:top-[15rem] bg-white rounded-tr-[4rem] py-8 px-8 flex flex-col md:block min-h-[calc(100vh-10rem)] md:min-h-auto md:w-3/5 mx-auto md:w-3/5 md:rounded-[4rem] md:py-10 md:px-14'>
            ...
            <div className='flex-1 p-4'>
                <h2 className='font-bold text-3xl mb-4 md:text-4xl md:mb-6'>Set Your PIN Code</h2>
                ...
                <div>
                    <p className='text-sky-900 md:text-xl'>Input your desired four digit pin</p>
                </div>
                {errorMessage && (
            ...
            <div>
                <button onClick={handleSubmit} type='button' className='w-full py-3 bg-sky-950 text-white rounded-3xl text-lg font-bold mb-6 cursor-pointer md:py-4 md:text-2xl md:tracking-wide md:mt-3'>Create PIN</button>
            </div>
        </div>
    </div>
  )
